  ...
  class RMTLD3_reader : public RTML_reader<T>
  {
    ...
    public:
      ...
      std::tuple<timespanw, Event< int >, size_t, bool> searchIndexForwardUntil(TraceIterator<T> &iterator, timespanw t) {
         ...
         * type is std::tuple<timespanw, Event< int >, size_t, bool>
         */
        auto event_find_tuple = std::accumulate(
          tmp_it.begin(),
          tmp_it.end(),
          std::make_tuple ( tmp_it.getCurrentAbsoluteTime(), Event< int >(), tmp_it.getIt(), false ),
            [t]( const std::tuple<timespanw, Event< int >, size_t, bool> a, Event< int > e )
            {
